	...
	<div id="modal-welcome">
		...
		<div id="module-content">
			...
			<div id="slide02" class="slide">
				<p class="main-text">Which of the two lessons will be more engaging?</p>

				<div class="lessons">
					<div class="lesson1">
						<p>An <br/>Introduction<br/> To</p>
						<p class="large">Gravity</p>
						<a href="#" id="btn-gravity">This One</a>
					</div>

					<div class="lesson2">
						<p>An <br/>Introduction<br/> To</p>
						<p class="large">Matter</p>
						<a href="#" id="btn-matter">This One</a>
					</div>
				</div>
			</div><!-- /#slide02 -->
			...
			<div id="slide03" class="slide">
				...
				<div class="blue-box">
					<p class="title">ANSWER: TRICK QUESTION</p>
					<p>Intuitively, we assume that some topics are inherently more engaging than others. But, in fact, it all depends on how you teach it.</p>
				</div>
			...
			<div id="slide05" class="slide">
				...
				<div class="blue-box">
					...
					<p>This teacher did not apply enagement techniques.<br>He averaged a “mostly not” from our team.</p>
			...
			<div id="slide07" class="slide">
				...
				<div class="blue-box">
					<p class="title">Feedback:</p>
					<p>This teacher applied multiple enagement techniques.<br>He averaged a “mostly” from our team.</p>
			...
			<div id="slide-8" class="slide">
				...
				<h2>Check which strategies you use to spark curiosity in your classroom.</h2>

				<div class="checkboxes">

					<form action="#" method="post" id="checkbox-results">
						...
						<input type="submit" value="Submit" id="intro-checkboxes" />
			...
			<div id="slide-9" class="slide">
				...
				<h2>From your data, we recommend starting with the following:</h2>
...
<script type="text/javascript">
	...
	function init(){
		...
		$('#lightbulbs a.lightbulb').mouseover(function(event){

			var current = $(this);
			// console.log(current.attr('id'));
		
			if($(current).attr('id') == 'btn-s'){
				$('#btn-s').css('background-position', '0 bottom');
			}
			if($(current).attr('id') == 'btn-p'){
				$('#btn-p').css('background-position', '0 bottom');
			}
			if($(current).attr('id') == 'btn-a'){
				$('#btn-a').css('background-position', '0 bottom');
			}
			if($(current).attr('id') == 'btn-r'){
				$('#btn-r').css('background-position', '0 bottom');
			}
			if($(current).attr('id') == 'btn-k'){
				$('#btn-k').css('background-position', '0 bottom');
			}

		});

		$('#lightbulbs a.lightbulb').mouseout(function(event){
		...
		$('#btn-sub-menu-close').click(function(){
			$('#sub-menu').animate({
				width: 0
			}, 500, function() {
				// Animation complete.
				$('#sub-menu').css("display", "none");
				$('a#btn-sub-menu-close').css("display", "none");
			});
		});

		//Slide 01
		$('#slide01 video').click(function(){
			$('#slide01 video').get(0).play();
		});
	...
	function slide_back(event){
		...
		if($(current).attr('id') == 'slide04'){
			// console.log('slide04');
			$(current).removeClass('active');
			$('#slide04').hide();

			$('#slide03').addClass('active');
			$('#slide03').show();
		}

		if($(current).attr('id') == 'slide05'){
			// console.log('slide05');
			$(current).removeClass('active');
			$('#slide05').hide();
		...
		if($(current).attr('id') == 'slide06'){
			// console.log('slide06');
			$(current).removeClass('active');
			$('#slide06').hide();

			$('#slide05').addClass('active');
			$('#slide05').show();
		}

		if($(current).attr('id') == 'slide07'){
			// console.log('slide07');
			$(current).removeClass('active');
			$('#slide07').hide();
		...
		if($(current).attr('id') == 'slide-8'){
			// console.log('slide08');
			$(current).removeClass('active');
			$('#slide-8').hide();

			$('#slide07').addClass('active');
			$('#slide07').show();
		}
	...
	function slide_next(event){
		...
		if($(current).attr('id') == 'slide01'){
			// console.log('slide01');
			// var video1 = $('#slide01 video');
			var video1 = $('#slide01 video').get(0);
			if (video1.paused) {
				//video1.play();
			}
			else{
		...
		if($(current).attr('id') == 'slide04'){
			// console.log('slide04');
			var video2 = $('#slide04 video').get(0);
			if (video2.paused) {
				//video1.play();
			}
			else{
		...
		if($(current).attr('id') == 'slide06'){
			// console.log('slide06');
			var video3 = $('#slide06 video').get(0);
			if (video3.paused) {
				//video1.play();
			}
			else{
	...
	function herp(event){
		...
		$.each(derp, function(index){
			...
			switch(skib) {
				...
				case "s=on":
					s = 1;
					console.log('s is on' + s);
					break;

				case "p=on":
					p = 1;
					console.log('p is on' + p);
					break;

				case "a=on":
					a = 1;
					console.log('a is on' + a);
					break;

				case "r=on":
					r = 1;
					console.log('r is on' + r);
					break;
...
<div id="bkg-modal-s">  
	<div id="modal-suprising">
	  	...
	  	<div id="module-content-s">
			...
			<div id="surprising04" class="slide">
			...
			<div id="surprising05" class="slide">
			...
			<div id="surprising06" class="slide">
			...
			<div id="surprising11" class="slide">
...
<script type="text/javascript">
	...
	function modal_open_s(event){
  	...
  	function slide_next_s(event){
  		...
  		if($(current).attr('id') == 'surprising01'){
			...
			$('#surprising02').addClass('active');
			$('#surprising02').show();
  		}

		if($(current).attr('id') == 'surprising02'){
			$(current).removeClass('active');
			$('#surprising02').hide();
		...
		if($(current).attr('id') == 'surprising03'){
			var video2 = $('#surprising03 video').get(0);
  			if (video2.paused) {
				//video2.play();
  			}else {
				video2.pause();
		...
		if($(current).attr('id') == 'surprising04'){
			$(current).removeClass('active');
			$('#surprising04').hide();
		...
		if($(current).attr('id') == 'surprising05'){
			$(current).removeClass('active');
			$('#surprising05').hide();
		...
		if($(current).attr('id') == 'surprising06'){
			$(current).removeClass('active');
			$('#surprising06').hide();
		...
		if($(current).attr('id') == 'surprising08'){
			$(current).removeClass('active');
			$('#surprising08').hide();
		...
		if($(current).attr('id') == 'surprising09'){
			$(current).removeClass('active');
			$('#surprising09').hide();
		...
		if($(current).attr('id') == 'surprising10'){
			$(current).removeClass('active');
			$('#surprising10').hide();
		...
		if($(current).attr('id') == 'surprising11'){
			$('#bkg-modal-s').hide();
  		 	$('#bkg-modal-s .slide').hide();
  		 	$('#bkg-modal-s .slide').removeClass('active');
  		 	$('#surprising01').addClass('active');
	...
	function select_active_s(){
		...
		if(current_id == 'surprising04'){
			$('a#tactic1-2').addClass('active');
		}

		if(current_id == 'surprising05'){
			$('a#tactic1-3').addClass('active');
		}

		if(current_id == 'surprising06'){
